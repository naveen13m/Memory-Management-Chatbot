...
void ChatLogic::AddAllTokensToElement(std::string tokenID, tokenlist &tokens, T &element)
{
    ...
    while (true)
    {
        token = std::find_if(token, tokens.end(),
                [&tokenID](const std::pair<std::string, std::string> &pair) { return pair.first == tokenID;; });
        if (token != tokens.end())
        {
...
//                            GraphEdge *edge = new GraphEdge(id);
                            ...
                            (*childNode)->AddEdgeToParentNode(edge.get()); // incoming edge to child node
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); // outgoing edge to parent node
                        }
    ...
    ChatBot chatBotLocal = ChatBot("../images/chatbot.png"); // creation on stack
